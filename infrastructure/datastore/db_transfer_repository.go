...
func NewDbTransferRepository(db *sqlx.DB) *dbTransferRepository {
	return &dbTransferRepository{db: db}
}

// 指定したタグ名に一致する送金情報を指定の件数取得
func (r *dbTransferRepository) GetInfosByTagNames(ctx context.Context, tagNames []string, infoNumber int) ([]*entity.TransferInfo, error) {
	// 条件に合う送金情報を取得
...
func (r *dbTransferRepository) GetPrevInfosByTagNames(ctx context.Context, tagNames []string, prevInfoID int64, infoNumber int) ([]*entity.TransferInfo, error) {
	// 条件に合う送金情報を取得
...
// 存在するすべてのタグを取得
func (r *dbTransferRepository) GetAllTags(ctx context.Context) ([]*entity.Tag, error) {
	var tags []*entity.Tag
...
func (r *dbTransferRepository) StoreInfo(ctx context.Context, info *entity.TransferInfo, tagNames []string) (int64, error) {
	// トランザクションを開始
...
func (r *dbTransferRepository) StoreChannelStatus(ctx context.Context, channelStatus *entity.TelegramChannel) error {
	// トランザクションを開始
	tx, err := r.db.BeginTxx(ctx, nil)
	if err != nil {
...
func (r *dbTransferRepository) UpdateChannelStatus(ctx context.Context, channelStatus *entity.TelegramChannel) error {
	// トランザクションを開始
	tx, err := r.db.BeginTxx(ctx, nil)
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
...
func (r *dbTransferRepository) GetChannelStatusByUsername(ctx context.Context, username string) (*entity.TelegramChannel, error) {
	// 取得した結果を格納するための変数を宣言
