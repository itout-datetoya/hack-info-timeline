...
type dbHackingRepository struct {
	db *sqlx.DB
}

// hackingRepository の新しいインスタンスを生成
func NewDbHackingRepository(db *sqlx.DB) *dbHackingRepository {
	return &dbHackingRepository{db: db}
}

// 指定したタグ名に一致する情報を指定の件数取得
func (r *dbHackingRepository) GetInfosByTagNames(ctx context.Context, tagNames []string, infoNumber int) ([]*entity.HackingInfo, error) {
	// 条件に合うハッキング情報を取得
...
func (r *dbHackingRepository) GetPrevInfosByTagNames(ctx context.Context, tagNames []string, prevInfoID int64, infoNumber int) ([]*entity.HackingInfo, error) {
	// 条件に合うハッキング情報を取得
...
// 存在するすべてのタグを取得
func (r *dbHackingRepository) GetAllTags(ctx context.Context) ([]*entity.Tag, error) {
	var tags []*entity.Tag
...
func (r *dbHackingRepository) StoreInfo(ctx context.Context, info *entity.HackingInfo, tagNames []string) (int64, error) {
	// トランザクションを開始
...
func (r *dbHackingRepository) StoreChannelStatus(ctx context.Context, channelStatus *entity.TelegramChannel) error {
	// トランザクションを開始
	tx, err := r.db.BeginTxx(ctx, nil)
	if err != nil {
...
func (r *dbHackingRepository) UpdateChannelStatus(ctx context.Context, channelStatus *entity.TelegramChannel) error {
	// トランザクションを開始
	tx, err := r.db.BeginTxx(ctx, nil)
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
...
func (r *dbHackingRepository) GetChannelStatusByUsername(ctx context.Context, username string) (*entity.TelegramChannel, error) {
	// 取得した結果を格納するための変数を宣言
